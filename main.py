...
async def enhance_photo(
    ...
    try:
        scan, meta = process_document(payload, scan_mode=scan_mode, auto_crop=False)
...
async def scan_document(
    ...
    """
    Full scan flow: auto-detect document, crop/deskew, then enhance.
    """
    if not file.content_type or not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="Uploaded file must be an image")
    ...
    try:
        scan, meta = process_document(payload, scan_mode=scan_mode, auto_crop=True)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=500, detail=f"Processing error: {exc}") from exc
    ...
    headers = {
        "X-Scan-Meta": json.dumps(meta, ensure_ascii=True),
        "Content-Disposition": f'inline; filename="scan_{file.filename or "document"}.png"',
    }
    return Response(content=encoded.tobytes(), media_type="image/png", headers=headers)
