...
# ----------------------------------------------------------------------
# Улучшенные детекторы (склеиваем элементы, чтобы найти целый лист)
# ----------------------------------------------------------------------

def _morph_preprocess(image_bgr: np.ndarray) -> np.ndarray:
    """Склеивает текст и линии в одну область с помощью морфологии."""
    gray = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY)
    # Адаптивная бинаризация для выявления всех не-белых элементов
    bin_img = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                    cv2.THRESH_BINARY_INV, 21, 8)
    # Закрываем промежутки между буквами и линиями
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (9, 9))
    closed = cv2.morphologyEx(bin_img, cv2.MORPH_CLOSE, kernel, iterations=2)
    # Расширяем, чтобы соединить близкие блоки
    dilated = cv2.dilate(closed, kernel, iterations=2)
    return dilated


def find_document_contour_hough(image_bgr: np.ndarray) -> np.ndarray | None:
    mask = _morph_preprocess(image_bgr)
    # Теперь ищем края уже на этой маске
...
# ----------------------------------------------------------------------
# Эталонная обрезка по всем не-белым пикселям (всегда корректна)
# ----------------------------------------------------------------------

def get_content_bbox(image: np.ndarray) -> tuple[int, int, int, int]:
    """Возвращает boundingRect всех пикселей, не являющихся почти белыми (порог 250)."""
...
def simple_crop_by_content(image_bgr: np.ndarray) -> np.ndarray:
    """Обрезает по boundingRect всех значимых пикселей."""
    x, y, w, h = get_content_bbox(image_bgr)
    return _apply_padding(image_bgr, x, y, w, h)


# ----------------------------------------------------------------------
# Основная функция обрезки с защитой от ложных контуров
# ----------------------------------------------------------------------
...
def crop_document_if_found(image_bgr: np.ndarray) -> tuple[np.ndarray, bool, str]:
    ...
    if points is not None:
        ...
        if max_angle_dev < 2.0 and side_ratio_ok:
            candidate = _apply_padding(image_bgr, *cv2.boundingRect(points.astype(np.int32)))
        else:
        ...
        # Проверяем, что обрезанный кандидат содержит почти всё содержимое
        orig_bbox = get_content_bbox(image_bgr)
        cand_bbox = get_content_bbox(candidate)
        orig_area = orig_bbox[2] * orig_bbox[3]
        cand_area = cand_bbox[2] * cand_bbox[3]
        if orig_area == 0 or cand_area / orig_area < 0.8:
            # Результат детектора неудовлетворителен – используем fallback
            return simple_crop_by_content(image_bgr), True, "fallback_content"
        return candidate, True, f"{detector}_straight_or_warp"

    # Контур не найден – fallback
    return simple_crop_by_content(image_bgr), True, "fallback_content"


# ----------------------------------------------------------------------
# Функции улучшения (без изменений)
# ----------------------------------------------------------------------
