...
def detect_document_corners(image_bgr: np.ndarray) -> tuple[np.ndarray | None, str]:
    ...
    mask = estimate_paper_mask(image_bgr, gray_norm)
    hard = cv2.threshold(mask, 128, 255, cv2.THRESH_BINARY)[1]
    ...
    if len(approx) == 4:
        return approx.reshape(4, 2).astype(np.float32), "approx_quad"
